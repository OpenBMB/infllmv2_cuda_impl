...
home_dir = "/user/xuxiaoyue/clap_stage1"
def test_stage1_cp_load(causal=True):
    ...
    cp1_k = []
    cp1_scores = []
    for rank in range(4): # 模拟在4个rank上的计算
    ...
    # 已知 rank 01, 23 分别属于一个tp group
    std_q = []
    std_k = []
    std_scores = []
    for rank in range(2): # [2, 256, 127]
    ...
    # 已知 rank 01, 23 分别属于一个cp group
    cp2_q = [[None] * 4 for _ in range(2)]
    cp2_k = [None] * 2
    cp2_scores = [[None] * 4 for _ in range(2)]
    for rank in range(4):
        cp_group = rank // 2
        cp_rank = rank % 2
    ...
    # cp2_scores[0] = torch.cat(cp2_scores[0], dim=0)
    # cp2_scores[1] = torch.cat(cp2_scores[1], dim=0)
    cp2_q[0] = torch.cat(cp2_q[0], dim=0)
    cp2_q[1] = torch.cat(cp2_q[1], dim=0)
    ...
    assert torch.allclose(std_q[1], cp2_q[1]), 'q应该一致'
    assert torch.allclose(std_k[0], cp2_k[0]), 'k应该一致'
    assert torch.allclose(std_k[1], cp2_k[1]), 'k应该一致'


    breakpoint()
    # std_scores[0] = [2, 256, 127]
    # cp2_scores[0] = [2, 64, 31], [2, 64, 63], [2, 64, 95], [2, 64, 127]
